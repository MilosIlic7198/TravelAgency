<template>
    <form>
        ...
        <div class="m-2">
            ...
            <input type="text" class="border border-gray-200 rounded p-2" name="title" v-model="newPassword" />
            <div class="form-text">Leave this empty if you don't want to reset password!</div>
        </div>
        <div class="form-check m-2">
            <input class="form-check-input" type="checkbox" value="Admin" id="adminCheck"
                :checked="roles[0] == 'Admin' || roles[1] == 'Admin' || roles[2] == 'Admin'">
            <label class="form-check-label" for="adminCheck">
                Admin
            </label>
        </div>
        <div class="form-check m-2">
            <input class="form-check-input" type="checkbox" value="Moderator" id="moderatorCheck"
                :checked="roles[0] == 'Moderator' || roles[1] == 'Moderator' || roles[2] == 'Moderator'">
        ...
        <div class="form-check m-2">
            <input class="form-check-input" type="checkbox" value="User" id="userCheck"
                :checked="roles[0] == 'User' || roles[1] == 'User' || roles[2] == 'User'">
...
export default {
    ...
    methods: {
        getPerson() {
            axios.get(`/api/get-person/${this.$route.params.id}`).then((res) => {
        ...
        save() {
            ...
            if (this.newPassword != "" && this.newPassword.length > 6) {
                editedData.append("password", this.newPassword);
            } else {
                editedData.append("password", this.person.password);
            }
