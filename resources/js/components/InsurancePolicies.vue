<template>
    ...
    <div v-else>
        <div class="border border-gray-200 rounded p-2 m-2" v-for="policy in policies">
            <p v-if="policy.type == 'Group'">
                <span><b>Type:</b> <a href="" @click="go(policy.id)">{{ policy.type }}</a></span>
            </p>
            <p v-else>
                <b>Type:</b> {{ policy.type }}
            </p>
            <p><b>Description:</b> {{ policy.description }}</p>
            <p><b>Holder Name:</b> {{ policy.holder_name }} {{ policy.holder_surname }}</p>
        ...
        axios.get("/api/get-all-policies").then((res) => {
            ...
            if (status == "200") {
                this.policies = res.data.map(policy => {
                    ...
                    policy.date_to = moment(policy.date_from).format("DD.MM.YYYY")
                    return policy;
                });
            }
        });
